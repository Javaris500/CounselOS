...
**There is no code yet.** The repository contains `docs/` and nothing else — no `apps/`, no `packages/`, no `package.json`, and no commits on `master`. Everything below describes the system as specified in `docs/`, which is the design the code must be built to. When a path like `apps/api/src/database/schema.ts` appears here, it is a target location, not an existing file.

Practical consequence: **never claim a file, table, endpoint, or command exists because this doc or a spec doc mentions it.** Check the filesystem. If the first task is scaffolding, scaffold to `docs/02-repo-structure.md` exactly.
...
3. **`instrument.ts` is the first import in `main.ts`** — Sentry must initialize before NestJS or bootstrap errors are lost.
...
5. **`rediss://`, not `redis://`.** TLS required.
...
Modules with extra structure: `documents/` (classifiers, processors, validators, `chunks.repository.ts`), `deadlines/` (extraction, superseding, urgency calculator, ics, schedulers), `queues/` (processors). `dashboard/` owns no table — it is pure aggregation over other modules' services.
...
- **Never log PII or response bodies.** Names, emails, phones mask to `[REDACTED]`.
- **The Supabase service key is scoped to two modules only.** Do not import it elsewhere.
...
|---|---|---|
| Unit | Nothing (mocked repo) | Deterministic logic: transitions, urgency tiers, TREC date math, token budget, magic bytes |
| Integration | Real Postgres + Redis (testcontainers), mocked external APIs | DB queries, queue jobs, cache behavior, soft-delete filters |
| E2E | Full HTTP stack + real JWT, mocked externals only | The module works through the real request lifecycle |
...
Visual design: `docs/07-design-handoff.md` + `docs/design-system-v5.html` (Design System v5 tokens → `styles/globals.css`).

---

## Commands

These are the commands the docs specify. **None of them work yet** — the workspace is not scaffolded. Node 20 LTS (`.nvmrc`), pnpm workspaces + turbo.
...
pnpm --filter api db:seed       # load the 5 Austin test fixtures
pnpm --filter api db:studio     # Drizzle Studio

pnpm test                       # all tests
pnpm --filter api test:unit     # fast, no I/O
pnpm --filter api test:int      # integration (testcontainers — needs Docker)
pnpm --filter api test:e2e      # E2E — the module gate
...
Env vars are listed in `docs/00-developer-guide.md` §4 and `docs/11-test-data.md`. `validateEnvVars()` runs at boot and **fails fast** — a missing env var must never surface as a runtime error three requests deep.
...
- **Known drift, already identified:** `04-data-contracts.md` and `10-tdd-guide.md` predate the Drizzle decision and still reference **Prisma** (and R2 for storage) — the stack is **Drizzle + Supabase Storage**. `10-tdd-guide.md`'s Jest config also uses `*.unit.spec.ts` / `*.integration.spec.ts`, while `01-codebase.md` and `02-repo-structure.md` use `<name>.service.spec.ts` / `<name>.repository.spec.ts` / `<name>.e2e-spec.ts` — the latter is the intended convention.
- **Docs change in the same PR as the code they describe.** A schema change without an `03-schema.md` update is an incomplete PR. `docs/README.md` is the index and the ships/doesn't-ship list; a root `README.md` derived from `00-developer-guide.md` is still owed.
