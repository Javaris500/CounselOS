...
- **Known drift, already identified:** `10-tdd-guide.md` predates the Drizzle decision and still references **Prisma** (and R2 for storage) — the stack is **Drizzle + Supabase Storage**. Its Jest config also uses `*.unit.spec.ts` / `*.integration.spec.ts`, while `01-codebase.md`, `02-repo-structure.md`, and the live `jest.config.ts` use `<name>.spec.ts` / `<name>.repository.spec.ts` / `<name>.e2e-spec.ts` — the latter is the intended convention. **`04-data-contracts.md` was on this list and has been fixed** (2026-08-16): Drizzle throughout, Supabase Storage, TimeEntry/Invoice corrected to Phase 1, `access_log` and `client_messages` documented as existing, client auth corrected to signed tokens rather than Supabase accounts.
